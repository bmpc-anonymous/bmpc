...
class Ensemble(nn.Module):
    ...
    def __init__(self, modules, **kwargs):
        ...
        # original ensemble impl, fit for torch<=2.3
        # fn, params, _ = combine_state_for_ensemble(modules)
        # self.vmap = torch.vmap(fn, in_dims=(0, 0, None), randomness='different', **kwargs)
        # self.params = nn.ParameterList([nn.Parameter(p) for p in params])

        # new ensemble impl, fit for torch>=2.3
        # replace combine_state_for_ensemble accoding to https://pytorch.org/docs/main/func.migrating.html
        base_model = copy.deepcopy(modules[0]).to("meta")
        params, buffers = torch.func.stack_module_state(modules)
        self._vmap_buffers = buffers
        self._vmap_keys = list(params.keys())

        def call_single_model(params, buffers, data):
            return torch.func.functional_call(base_model, (params, buffers), (data,))

        self.vmap = torch.vmap(
            call_single_model, in_dims=(0, 0, None), randomness="different", **kwargs
        )
    ...
    def forward(self, *args, **kwargs):
        return self.vmap(
            {key: value for key, value in zip(self._vmap_keys, self.params)},
            self._vmap_buffers,
            *args,
            **kwargs,
        )
