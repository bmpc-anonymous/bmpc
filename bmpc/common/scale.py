...
class RunningScale:
	...
	def __init__(self, cfg, min_scale=1.):
		self.cfg = cfg
		self.min_scale = min_scale
		self._device = cfg.device
		self._value = torch.ones(1, dtype=torch.float32, device=torch.device(self._device))
		self._percentiles = torch.tensor([5, 95], dtype=torch.float32, device=torch.device(self._device))
