...
class WorldModel(nn.Module):
	...
	def __init__(self, cfg):
		...
		if self.cfg.use_v_instead_q:
			init.zero_([self._reward[-1].weight, self._Vs.params[-2]])
		else:
		...
		self.expl_log_std_min = torch.tensor(cfg.expl_log_std_min)
		self.expl_log_std_dif = torch.tensor(cfg.expl_log_std_max) - self.expl_log_std_min
	...
	def to(self, *args, **kwargs):
		...
		self.expl_log_std_min = self.expl_log_std_min.to(*args, **kwargs)
		self.expl_log_std_dif = self.expl_log_std_dif.to(*args, **kwargs)
		return self
	...
	def pi(self, z, task, expl=False):
		...
		expl: whether to use expl_log_std_min and expl_log_std_dif
		"""
		if self.cfg.multitask:
			z = self.task_emb(z, task)
		...
		mu = torch.tanh(mu)
		pi = (mu + eps * log_std.exp()).clamp(-1,1)
	...
	def Q(self, z, a, task, return_type='min', target=False):
		...
		"""
		if self.cfg.use_v_instead_q:
			return self.V(z, task, return_type, target)
	...
	def V(self, z, task, return_type='min', target=False):
		...
		V1, V2 = out[np.random.choice(self.cfg.num_q, 2, replace=False)]
		V1, V2 = math.two_hot_inv(V1, self.cfg), math.two_hot_inv(V2, self.cfg)
		return torch.min(V1, V2) if return_type == 'min' else (V1 + V2) / 2
