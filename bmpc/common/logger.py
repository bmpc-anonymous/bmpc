...
class Logger:
    """logging object by origin TD-MPC2 Code. Logs either locally or using wandb."""

    """not supported in BMPC"""
...
class TBLogger:
    """Logging object in BMPC. Logs using tensorboard."""
    ...
    def __init__(self, cfg):
        ...
        # reduce the tensorboard historgram logs number
        self.tb_histogram_interval = 5
        self.tb_histogram_count = 0
    ...
    def _write_tb(self, d, category):
        ...
        for k, v in d.items():
            ...
            elif k in TENSORBOARD_LOG_CLASSES["plan_metrics"]:
                self._writer.add_scalar("plan_metrics/{}".format(k), v, d["step"])
            elif k in TENSORBOARD_LOG_CLASSES["plan_histograms"]:
                if self.tb_histogram_count % self.tb_histogram_interval == 0:
                    self._writer.add_histogram("plan_histograms/{}".format(k), v, d["step"])
            else:
                self._writer.add_scalar("train_metrics/{}".format(k), v, d["step"])
        if d.get("value_max_pi_diff") != None:
            self.tb_histogram_count += 1
        self._writer.flush()
    ...
    def log(self, d, category="train"):
        ...
        if category in CAT_TO_COLOR.keys():
            self._print(d, category)
        if category in ["train", "eval", "eval2", "eval3"]:
